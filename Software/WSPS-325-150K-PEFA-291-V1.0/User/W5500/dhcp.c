...
uint8 check_DHCP_state(SOCKET s) 
{
	...
	{
		if ((len = getSn_RX_RSR_TOP(s)) > 0)							/*接收到数据*/
		{
	...
	else																								/*socket处于关闭状态，重新初始化socket*/
	{
		...
		if(!socket_top(0,Sn_MR_UDP,DHCP_CLIENT_PORT,0x00))/*初始化socket和端口*/
		{ 
...
{
	uint8 dhcpret=0;
	dhcpret = check_DHCP_state(0);             /*获取DHCP服务状态*/
	...
	switch(dhcpret)
	{
		...
		case DHCP_RET_UPDATE:														 /*成功获取到IP地址*/ 
		  dhcp_ok=1;                  
			ip_for_dhcp();                                /*将获取到的IP地址写入W5500寄存器*/ 
	    break;
