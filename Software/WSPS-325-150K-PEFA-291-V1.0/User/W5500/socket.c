...
uint8 socket(SOCKET s, uint8 protocol, uint16 port, uint8 flag)
{
        ...
        ((protocol&0x0F) == Sn_MR_PPPOE)   )
    {
        ...
        IINCHIP_WRITE( Sn_IR(s) , 0xFF);	 /* 关闭socket */
...
uint8 socket_top(SOCKET s, uint8 protocol, uint16 port, uint8 flag)
{
    uint8 ret;
    if(((protocol&0x0F) == Sn_MR_TCP)     ||
        ((protocol&0x0F) == Sn_MR_UDP)    ||
        ...
        ((protocol&0x0F) == Sn_MR_PPPOE))
    {
        ...
        IINCHIP_WRITE_TOP( Sn_CR(s) ,Sn_CR_CLOSE);
        while( IINCHIP_READ_TOP(Sn_CR(s) ) )
                ;/* ------- */
        IINCHIP_WRITE_TOP( Sn_IR(s) , 0xFF);	 /* all clear */	 
        
        
        IINCHIP_WRITE_TOP(Sn_MR(s) ,protocol | flag);
			
      if (port != 0) {
         IINCHIP_WRITE_TOP( Sn_PORT0(s) ,(uint8)((port & 0xff00) >> 8));
         IINCHIP_WRITE_TOP( Sn_PORT1(s) ,(uint8)(port & 0x00ff));
      } 
//			else {  
//         local_port++; // if don't set the source port, set local_port number.
//         IINCHIP_WRITE_TOP(Sn_PORT0(s) ,(uint8)((local_port & 0xff00) >> 8));
//         IINCHIP_WRITE_TOP(Sn_PORT1(s) ,(uint8)(local_port & 0x00ff));
//      }
			
      IINCHIP_WRITE_TOP( Sn_CR(s) ,Sn_CR_OPEN);   // run sockinit Sn_CR
...
uint8 listen_TOP(SOCKET s)
{
   uint8 ret;
   if (IINCHIP_READ_TOP( Sn_SR(s) ) == SOCK_INIT)
   {
      IINCHIP_WRITE_TOP( Sn_CR(s) ,Sn_CR_LISTEN);
      /* wait to process the command... */
      while( IINCHIP_READ_TOP(Sn_CR(s) ) )
         ;
      /* ------- */
...
uint8 connect_Top(SOCKET s, uint8 * addr, uint16 port)
{
    uint8 ret;
    if
        (
    ...
    else
    {
        ret = 1;
        // set destination IP
        IINCHIP_WRITE_TOP( Sn_DIPR0(s), addr[0]);
        IINCHIP_WRITE_TOP( Sn_DIPR1(s), addr[1]);
        IINCHIP_WRITE_TOP( Sn_DIPR2(s), addr[2]);
        IINCHIP_WRITE_TOP( Sn_DIPR3(s), addr[3]);
        IINCHIP_WRITE_TOP( Sn_DPORT0(s), (uint8)((port & 0xff00) >> 8));
        IINCHIP_WRITE_TOP( Sn_DPORT1(s), (uint8)(port & 0x00ff));
        IINCHIP_WRITE_TOP( Sn_CR(s) ,Sn_CR_CONNECT);
        /* wait for completion */
        while ( IINCHIP_READ_TOP(Sn_CR(s) ) ) ;

        while ( IINCHIP_READ_TOP(Sn_SR(s)) != SOCK_SYNSENT )
        {
            if(IINCHIP_READ_TOP(Sn_SR(s)) == SOCK_ESTABLISHED)
            {
                break;
            }
//            if (getSn_IR(s) & Sn_IR_TIMEOUT)
            if (IINCHIP_READ_TOP(Sn_IR(s)) & Sn_IR_TIMEOUT)							
            {
                IINCHIP_WRITE_TOP(Sn_IR(s), (Sn_IR_TIMEOUT));  // clear TIMEOUT Interrupt
                ret = 0;
                break;
            }
        }
...
uint16 send_TOP(SOCKET s, const uint8 * buf, uint16 len)
{
  uint8 status=0;
  uint16 ret=0;
  uint16 freesize=0;
  ...
  do
  {
    freesize = getSn_TX_FSR_TOP(s);
    status = IINCHIP_READ_TOP(Sn_SR(s));
    if ((status != SOCK_ESTABLISHED) && (status != SOCK_CLOSE_WAIT))
    {
      ret = 0;
      break;
  ...
  // copy data
  send_data_processing_top(s, (uint8 *)buf, ret);
  IINCHIP_WRITE_TOP( Sn_CR(s) ,Sn_CR_SEND);

  /* wait to process the command... */
  while( IINCHIP_READ_TOP(Sn_CR(s) ) );

  while ( (IINCHIP_READ_TOP(Sn_IR(s) ) & Sn_IR_SEND_OK) != Sn_IR_SEND_OK )
  {
    status = IINCHIP_READ_TOP(Sn_SR(s));
    if ((status != SOCK_ESTABLISHED) && (status != SOCK_CLOSE_WAIT) )
    {
  ...
  IINCHIP_WRITE_TOP( Sn_IR(s) , Sn_IR_SEND_OK);
...
uint16 recv_TOP(SOCKET s, uint8 * buf, uint16 len)
{
   uint16 ret=0;
   if ( len > 0 )
   {
      recv_data_processing_TOP(s, buf, len);
      IINCHIP_WRITE_TOP( Sn_CR(s) ,Sn_CR_RECV);
      /* wait to process the command... */
      while( IINCHIP_READ_TOP(Sn_CR(s) ));
      /* ------- */
      ret = len;
...
uint16 sendto_top(SOCKET s, const uint8 * buf, uint16 len, uint8 * addr, uint16 port)
{
   uint16 ret=0;
   ...
   else
   {
     ...
     while( (IINCHIP_READ_TOP( Sn_IR(s) ) & Sn_IR_SEND_OK) != Sn_IR_SEND_OK )
     {
      ...
      {
            /* clear interrupt */
      IINCHIP_WRITE_TOP( Sn_IR(s) , (Sn_IR_SEND_OK | Sn_IR_TIMEOUT)); /* clear SEND_OK & TIMEOUT */
      return 0;
      }
     }
      IINCHIP_WRITE_TOP( Sn_IR(s) , Sn_IR_SEND_OK);
...
uint16 recvfrom(SOCKET s, uint8 * buf, uint16 len, uint8 * addr, uint16 *port) 
{
   ...
   if ( len > 0 )
   {
        ...
        {
            case Sn_MR_UDP :
                wiz_read_buf(addrbsb, head, 0x08);                 /* 向固定地址读取数据 */  
                ptr += 8;
                
                addr[0]  = head[0];																 /* 读取目标IP */
                addr[1]  = head[1];
                addr[2]  = head[2];
                addr[3]  = head[3];
                *port    = head[4];                                /* 读取目标端口号 */
                *port    = (*port << 8) + head[5];
                ...
                data_len = head[6];
                data_len = (data_len << 8) + head[7];              /* 读取数据长度 */
...
uint16 recvfrom_top(SOCKET s, uint8 * buf, uint16 len, uint8 * addr, uint16 *port) 
{
    uint8 head[8]={0,0,0,0,0,0,0,0};
    uint16 data_len=0;
    uint16 ptr=0;
    ...
    if ( len > 0 )
    {
        ptr     = IINCHIP_READ_TOP(Sn_RX_RD0(s) );                             /*  */
        ptr     = ((ptr & 0x00ff) << 8) + IINCHIP_READ_TOP(Sn_RX_RD1(s));      /*  */
        addrbsb = (uint32)(ptr<<8) +  (s<<5) + 0x18;
        
        switch (IINCHIP_READ_TOP(Sn_MR(s) ) & 0x07)
        {
            case Sn_MR_UDP :
                wiz_read_buf_top(addrbsb, head, 0x08);                 /* 向固定地址读取数据 */  
                ptr += 8;
                ...
                *port    = (*port << 8) + head[5];
                ...
                wiz_read_buf_top(addrbsb, buf, data_len);                
                ptr += data_len;
                IINCHIP_WRITE_TOP( Sn_RX_RD0(s), (uint8)((ptr & 0xff00) >> 8));
                IINCHIP_WRITE_TOP( Sn_RX_RD1(s), (uint8)(ptr & 0x00ff));
                break;
            default :
                break;
      }
