...
void adc_data(uint16_t num1, uint16_t num2, uint16_t num3)
{
    ...
    System.Pout = (0.0117 * data1 * data1 + 0.4095 * data1 + 30.967) * System.K_value / 1000;
    
    System.Pfout = (0.0117 * data2 * data2 + 0.4095 * data2 + 30.967) * 1000 / 1000;
    
    if (data3 > 1000)
        System.Pin = (0.00036 * data3 * data3 + 0.113 * data3) * 1000 / 20000;
    else
        System.Pin = (0.00042 * data3 * data3 + 0.053 * data3) * 1 / 20000;
...
void POWER_calculate(void)
{
    ...
    {
        date = max[0] / 4;
        System.Voltage = date / 4096 * 56.52;

        for (i = 1; i < 5; i++)
        {
            date = max[i] / 4;
            System.Current[i - 1] = date * 3.3 / 4096 * 80;
        }
