...
{   
    wiz_write_buf_top(SUBR0, addr, 4);
}
/**
...
{
    wiz_write_buf_top(SIPR0, addr, 4);  
}
...
{
   return IINCHIP_READ_TOP(Sn_SR(s));
}
...
uint16 getSn_TX_FSR_TOP(SOCKET s)
{
    uint16 val=0,val1=0;
    do
    {
        val1 = IINCHIP_READ_TOP(Sn_TX_FSR0(s));
        val1 = (val1 << 8) + IINCHIP_READ_TOP(Sn_TX_FSR1(s));
        if (val1 != 0)
        {
            val = IINCHIP_READ_TOP(Sn_TX_FSR0(s));
            val = (val << 8) + IINCHIP_READ_TOP(Sn_TX_FSR1(s));
        }
    } while (val != val1);
    return val;
}
...
uint16 getSn_RX_RSR_TOP(SOCKET s)
{
    uint16 val=0,val1=0;
    do
    {
        val1 = IINCHIP_READ_TOP(Sn_RX_RSR0(s));
        val1 = (val1 << 8) + IINCHIP_READ_TOP(Sn_RX_RSR1(s));
        if(val1 != 0)
        {
            val = IINCHIP_READ_TOP(Sn_RX_RSR0(s));
            val = (val << 8) + IINCHIP_READ_TOP(Sn_RX_RSR1(s));
        }
    } while (val != val1);
    return val;
}
...
void send_data_processing_top(SOCKET s, uint8 *data, uint16 len)
{
    uint16 ptr =0;
    uint32 addrbsb =0;
    if(len == 0)
    ...
    ptr = IINCHIP_READ_TOP( Sn_TX_WR0(s) );
    ptr = ((ptr & 0x00ff) << 8) + IINCHIP_READ_TOP(Sn_TX_WR1(s));
    
    addrbsb = (uint32)(ptr<<8) + (s<<5) + 0x10;
    wiz_write_buf_top(addrbsb, data, len);
    
    ptr += len;
    IINCHIP_WRITE_TOP( Sn_TX_WR0(s) ,(uint8)((ptr & 0xff00) >> 8));
    IINCHIP_WRITE_TOP( Sn_TX_WR1(s),(uint8)(ptr & 0x00ff));
}
/**
...
void recv_data_processing_TOP(SOCKET s, uint8 *data, uint16 len)
{
    uint16 ptr = 0;
    uint32 addrbsb = 0;
    ...
    addrbsb = (uint32)(ptr<<8) + (s<<5) + 0x18;
    wiz_read_buf_top(addrbsb, data, len);
    ptr += len;
...
void socket_buf_init_top( uint8 * tx_size, uint8 * rx_size  )
{
    int16 i;
    int16 ssum=0,rsum=0;
    ...
    for (i = 0 ; i < MAX_SOCK_NUM; i++)       // Set the size, masking and base address of Tx & Rx memory by each channel
    {
        IINCHIP_WRITE_TOP( (Sn_TXMEM_SIZE(i)), tx_size[i]);
        IINCHIP_WRITE_TOP( (Sn_RXMEM_SIZE(i)), rx_size[i]);
